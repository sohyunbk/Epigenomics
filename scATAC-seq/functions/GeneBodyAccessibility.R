...
GeneBodyAccessibility <- function(obj, FeatureName="gene", nRange=500){
    ...
    ## Broaden annotation
    Start_new <- c((ranges(Ann)+nRange)@start)-1 ## it minus nRange from the start : -1 means to convert to the bed format coordinate
    Width_new <- c((ranges(Ann)+nRange)@width)+1 ## it add the end range with nRange : +1 means to convert to the bed format coordinate
