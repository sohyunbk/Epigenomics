...
smooth.data        <- function(x,
  ...
  data.use <- x
  #print(dim(data.use))
  #print(rownames(data.use))
  #print(colnames(data.use))
  # verbose
  if(!is.null(rds)){
  ...
  colnames(A) <- colnames(data.use)
  saveRDS(A,paste0(output_dir,'/',output,".MarkovMatrix.rds"))
  # smooth data
...
plot.act.scores    <- function(df,output_dir,
  ...
  for (i in 1:length(gids)){
    ...
    # plot
    # here you should change the name
...
runMajorPriori     <- function(all.b,
  ...
  # line-up ids
  colnames(activity) <- colnames(all.activity)
