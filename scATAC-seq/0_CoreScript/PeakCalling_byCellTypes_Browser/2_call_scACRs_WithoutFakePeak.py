...
def write_pooled_bed(meta_data, bed_file, column_name, output_base_name, output_dir, cores):
...
def sub_func_macs2(i, dict_file_name, output_dir):
    ...
    for bed_f, out_f in zip(read_bed_files, output_file_names):
        ...
        subprocess.run([generate_macs2_command], shell=True, check=True) # should remove #

        print("Done Running MACS2 Calls")



def run_macs2_threaded(dict_file_name, output_directory, cores):
...
def generate_normalized_bw(file_dict, reference_file, output_dir):
    ...
    for cell_type, file_list in file_dict.items():
        ...
        try:
            subprocess.run(
                [f"~/UCSC/bedSort {bed_graph_file} {bed_graph_file_sorted}"],
                shell=True,
                check=True,
            )
        except ValueError:
        ...
        try:
            generate_bedgraph_to_bw = f"~/UCSC/bedGraphToBigWig {bed_graph_file_normalized} {reference_file} {bw_file_name}"
...
def filter_peaks_for_fragments(peak_file, tn5_file): ## Added function.
    """
...
if __name__ == "__main__":
    ...
    pooled_bed_file_dict = write_pooled_bed(                   #Should remove #
        meta_file, args.bed, args.col, args.o, args.od, cores  #Should remove #
    )                                                           #Should remove #


    ## Call Peaks using MACS2
    called_macs2_peaks = run_macs2_threaded(pooled_bed_file_dict, args.od, cores) #Should remove #
