...
lgd =Legend(labels =levels(as.factor(anno$group)), 
            legend_gp = gpar(fill = 1:6), title = "CellCycle")

lgd =Legend(labels =levels(as.factor(anno$group)), 
            legend_gp = gpar(fill = 1:5), title = "CellCycle")

pdf("Heatmap_Average_z_CellCycle_All_group_V3.pdf", width =30, height=5, onefile=FALSE) 
...
ggsave("stackedbarchart_CellCycle_AnnV3.pdf", width=7, height=5)	
