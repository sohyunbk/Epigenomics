...
DevideSampleIntoFour <- function(StringVector){
  nSample <- length(StringVector)
  RandomSampleNumber <- sample(1:nSample, replace=F)
  ChooseNumber <- round(nSample/4)
  Re1 <- StringVector[RandomSampleNumber[1:ChooseNumber]]
  Re2 <- StringVector[RandomSampleNumber[ChooseNumber:(ChooseNumber*2)]]
  Re3 <- StringVector[RandomSampleNumber[(ChooseNumber*2):(ChooseNumber*3)]]
  Re4 <- StringVector[RandomSampleNumber[(ChooseNumber*2):nSample]]
  return(list(Re1,Re2,Re3,Re4))
}
...
All_A619 <- c(A619,A619_2)
#StringVector <- All_Bif3
Bif3 <- CellNames[endsWith(CellNames, 'bif3')]
Bif3_2 <- CellNames[endsWith(CellNames, 'bif3_2')]
All_Bif3 <- c(Bif3,Bif3_2)

A619_list <- DevideSampleIntoFour(All_A619)
A619_1_Re1 <- A619_list[[1]]
A619_1_Re2 <- A619_list[[2]]
A619_2_Re1 <- A619_list[[3]]
A619_2_Re2 <- A619_list[[4]]

Bif3_list <- DevideSampleIntoFour(All_Bif3)
Bif3_1_Re1 <- Bif3_list[[1]]
Bif3_1_Re2 <- Bif3_list[[2]]
Bif3_2_Re1 <- Bif3_list[[3]]
Bif3_2_Re2 <- Bif3_list[[4]]
