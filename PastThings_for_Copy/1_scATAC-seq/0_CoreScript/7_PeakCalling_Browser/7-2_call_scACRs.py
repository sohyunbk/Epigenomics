...
def read_meta_file(meta_file):
    """TODO: Docstring for read_meta_file.
    :returns: TODO
...
def rip_grep_split(i, bed_file_name, rep_1_dict, rep_2_dict, cell_file_names,output_dir):
    """TODO: Docstring for rip_grep_split.
    ...
    tmp_file_1 = output_dir + i + "_sampled_cells_rep1.txt"
    tmp_file_2 = output_dir + i + "_sampled_cells_rep2.txt"
    tmp_pool_file = output_dir + i + "_sampled_cells_pool.txt"
...
def write_pooled_bed(meta_data, bed_file, column_name, output_base_name, output_dir):
    def generate_cellID_celltype_dict(pandas_df, column_name):   #Geneartes a dictionary where key is cellID from meta data and value is the annotation label.
     #  :returns: dictionary where the cell ID is the key and the annotation is the value
    ...
    print("Preparing to Split Bed file By Cell Type Using 15 Cores")
    ...
    ### Simple Working Split. SLOW
    #grabbed_dict = simple_split(
    #    bed_file,
    #    cellID_celltype_dict,
    #    cellID_annotation_type_rep_1_dictionary,
    #    cellID_annotation_type_rep_2_dictionary,
    #)
    #write_output_bed_pools_reps(grabbed_dict, cell_type_file_name)
    return cell_type_file_name


def sub_func_macs2(i, dict_file_name, output_dir):
    """Call MACS2 on the Split Bulk Datasets.
    ...
    for bed_f, out_f in zip(read_bed_files, output_file_names):
        ...
        if output_dir == None:
            generate_macs2_command = f"macs2 callpeak -t {bed_f} -f BED --nomodel \
...
def run_macs2_threaded(dict_file_name, output_directory):
    """TODO: Docstring for run_macs2_threaded.
    :returns: TODO
    ...
    with Pool(10) as pool:
        m = pool.map(
            partial(
                sub_func_macs2, ## Run macs2 function!!
...
def generate_normalized_bw(file_dict, reference_file, output_dir):
    ...
    for cell_type, file_list in file_dict.items():
        ...
        try:
            subprocess.run(
                [f"sort -k1,1 -k2,2n {bed_graph_file} > {bed_graph_file_sorted}"],
                shell=True,
                check=True,
            )
        except ValueError:
...
def take_reproducible_peaks_shuffle(file_dict, index_fai_file, output_dir):
    ...
    for cell_type, file_list in file_dict.items():
        tn5_file = read_bed_file(file_list[0])
        ...
        # elif output_dir != None:
        #    pool_peak_file = read_bed_file(output_dir + "/" + file_list[3] + "_peaks.narrowPeak")
        #    rep1_peak_file = read_bed_file(output_dir + "/" + file_list[4] + "_peaks.narrowPeak")
        #    rep2_peak_file = read_bed_file(output_dir + "/" + file_list[5] + "_peaks.narrowPeak")
        #
        #    reproducible_peaks = output_dir + "/" + file_list[3].replace(".pool.macs", ".reproducible_narrow_peaks")
        #    null_shuffle_peaks = output_dir + "/" + file_list[3].replace(".pool.macs", ".null_shuffle")
        #    repro_integration = output_dir + "/" + file_list[3].replace(".pool.macs", ".integration.tn5")
        #    null_shuffle_peaks_inter = output_dir + "/" + file_list[3].replace(".pool.macs", ".null.integration.tn5")

        print(f"Generating Reproducible Peaks for {cell_type}")
        reproducible_peaks_local = (
            pool_peak_file.intersect(rep1_peak_file, u=True, f=0.1)
        ...
        pooled_summit_peaks.intersect(reproducible_peaks_local, u=True).saveas(
            reproducible_summits
        )
...
if __name__ == "__main__":
    ...
    #pooled_bed_file_dict = write_pooled_bed(
    #    meta_file, bed_file, args.col, args.o, args.od
    #)


    ## Split bed file and write output
    pooled_bed_file_dict = write_pooled_bed(
        meta_file, args.bed, args.col, args.o, args.od
    )


    ## Call Peaks using MACS2
    called_macs2_peaks = run_macs2_threaded(pooled_bed_file_dict, args.od)
    ...
    threhold_passed_beds = eFDR_process(reproducible_null, 0.001, args.od)
    ...
    merged_cleaned_bed_files = cated_bed_files_cleaned.merge(c=4, o="collapse").each(remove_invaid_merge).saveas()

    print(f"Finishing Up Peak Identification")
    generate_final_peak_set = remove_overlapping_peaks(merged_cleaned_bed_files) ## This function should be fixed
    ...
    pybedtools.BedTool(numbered_acr_peaks_final).saveas(output_file_name)
