...
DataName <- "WT_Re1"
InputFile <- "/scratch/sb14489/4.scRNAseq/2.snRNA-seq/2.Mapped_starsolo/WT_Replicate1Solo.out/GeneFull/filtered/"
...
compress_if_not_gz <- function(file) {
  if (!grepl("\\.gz$", file)) {
    ...
    message(paste(file, "compressed to", paste0(file, ".gz")))
  } else {
